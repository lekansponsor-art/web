...
Before the test suite is run the database is seeded with data. After the test suite has finished a [`deleteMany`](/orm/v7/reference/prisma-client/model-queries#deletemany) is used to clear the database of its data.
