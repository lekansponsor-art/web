...
> See the [model query options](/orm/v7/reference/prisma-client/model-query-options) reference for more information about the different types available.
...
This query will also result in an error if an author with an `id` of `1` does not exist. In this case, Prisma Client will give a more descriptive error message. You can also use the [`connectOrCreate`](/orm/v7/reference/prisma-client/nested-queries#connectorcreate) API to safely create a new user if one does not already exist with the given `id`.
