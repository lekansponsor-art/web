...
- [Nested reads](#nested-reads) (sometimes referred to as _eager loading_) via [`select`](/orm/v7/reference/prisma-client/model-query-options#select) and [`include`](/orm/v7/reference/prisma-client/model-query-options#include)
...
- Use [`include`](/orm/v7/reference/prisma-client/model-query-options#include) to include related records, such as a user's posts or profile, in the query response.
- Use a nested [`select`](/orm/v7/reference/prisma-client/model-query-options#select) to include specific fields from a related record. You can also nest `select` inside an `include`.
...
- Use a nested [`create`](/orm/v7/reference/prisma-client/model-queries#create) query
- Use a nested [`createMany`](/orm/v7/reference/prisma-client/nested-queries#nested-createmany-options) query

In most cases, a nested `create` will be preferable unless the [`skipDuplicates` query option](/orm/v7/reference/prisma-client/nested-queries#nested-createmany-options) is required. Here's a quick table describing the differences between the two options:
...
The following query uses nested [`create`](/orm/v7/reference/prisma-client/model-queries#create) to create:
...
The following query uses a nested [`createMany`](/orm/v7/reference/prisma-client/model-queries#createmany) to create:
...
The following query creates ([`create`](/orm/v7/reference/prisma-client/model-queries#create) ) a new `User` record and connects that record ([`connect`](/orm/v7/reference/prisma-client/nested-queries#connect) ) to three existing posts:
...
You can [`connect`](/orm/v7/reference/prisma-client/nested-queries#connect) an existing record to a new or existing user. The following query connects an existing post (`id: 11`) to an existing user (`id: 9`)
...
If a related record may or may not already exist, use [`connectOrCreate`](/orm/v7/reference/prisma-client/nested-queries#connectorcreate) to connect the related record:
...
To [`disconnect`](/orm/v7/reference/prisma-client/nested-queries#disconnect) _all_ related records in a one-to-many relation (a user has many posts), `set` the relation to an empty list as shown:
