...
Prisma is a TypeScript ORM. You describe your tables in a schema file, and Prisma generates the migrations and a typed query client from it. [Prisma 8](https://www.prisma.io/docs/orm) adds a Supabase extension that moves the two things an ORM on Supabase always left to hand-written SQL into that same file: the foreign keys into `auth.users` and your Row Level Security policies.

What you get:

- **One file for the whole model.** Tables, `auth.users` relations, and RLS policies live next to each other. Prisma writes the migration, policies included, and Postgres enforces every rule.
...
Three things in this file were impossible in a Prisma schema before:
...
Supabase's model is the right one: put the rules in Postgres and let RLS enforce them. The three problems below are not with RLS. They are with where the rules live and how your code picks a key.
...
Teams that add a CI check for stale types have hit generated output that [changes between runs](https://github.com/supabase/postgres-meta/issues/959) with no schema change. Relationships that PostgREST resolves have shown up [untyped](https://github.com/supabase/cli/issues/736), and `Json` columns have produced the ["type instantiation is excessively deep"](https://github.com/supabase/supabase-js/issues/808) error for years.

**With Prisma 8**, `contract emit` produces the TypeScript types from the same file that produced the migration. There is no second generator to forget.
...
| Types                     | `supabase gen types`, run after every migration          | `contract emit`, from the same file as the migration                         |
...
Auth flows in the browser, Storage, Realtime, and anything that runs inside Supabase Edge Functions. Prisma talks to Postgres directly over your project's session pooler from Node.js or Bun, and runs alongside `supabase-js`, not instead of it. Sign the user in with `supabase-js`, and pass the session's access token to `db.asUser()` on the server.

The current boundaries: the `auth.users` relation is a constraint, not something you can `include`; Node.js and Bun only; policy expressions are strings that Prisma migrates and reads back but does not type-check. The [extension guide](https://www.prisma.io/docs/orm/extensions/supabase#current-limitations) has the full list.
